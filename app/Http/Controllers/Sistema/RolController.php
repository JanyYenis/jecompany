...
class RolController extends Controller
{
    ...
    {
        $this->middleware('permission:rol.ver | rol.crear | rol.editar | rol.eliminar', ['only' => ['index']]);
        $this->middleware('permission:rol.crear', ['only' => ['store', 'create']]);
    ...
    public function asignarRol(Request $request, Usuario $usuario)
    {
        ...
        $usuariosNotificacion = Usuario::with('roles')->whereHas('roles', function($query){
            $query->whereIn('name', [Usuario::ROL_SUPER_ADMINISTRADOR, Usuario::ROL_ADMINISTRADOR]);
        })
        ->get();
        
        foreach ($usuariosNotificacion as $usuarioN) {
            Notification::sendNow($usuarioN, new UsuarioRolNotification($usuario));
            broadcast(new UsuarioRolEvent($usuarioN));
        }
